...
@section('content')
    ...
    <div class="p-2 overflow-y-auto" style="max-height: calc(100vh - 120px);">
        ...
        @empty($ordenSig)
            {{-- Usa @empty/@else (maneja variable no definida y null) y dentro evita que “0” se oculte. --}}
            <table class="border-0 mb-[4px] w-full">
                ...
                <tbody class="bg-white text-black">
                    <tr class="text-sm">
                        <td class="p-2">
                            <p>NO SE HA ENCONTRADO OTRA ORDEN PARA ESTE TELAR.</p>
                        </td>
                    </tr>
                </tbody>
            </table>
        @else
            <table class="border-0 mb-[4px] w-full">
                ...
                <tbody class="bg-white text-black">
                    <tr class="text-sm">
                        <td class="p-2">
                            {{-- Orden: no ocultar 0 y evitar error si es null --}}
                            <b>Orden:</b>
                            {{ ($op = $ordenSig->Orden_Prod) === null ? '' : decimales($op) }}

                            <br><b>Artículo + Tamaño:</b>
                            {{ preg_replace('/^[^-]*-\s*/', '', (string) ($ordenSig->Nombre_Producto ?? '')) }}
                        </td>
                        ...
                        <td class="p-2">
                            <b>Pedido:</b>
                            {{ isset($ordenSig->Entrega) ? formatearFecha($ordenSig->Entrega) : '' }}

                            <br><b>Fecha Inicio:</b>
                            {{ isset($ordenSig->Inicio_Tejido) ? formatearFecha($ordenSig->Inicio_Tejido) : '' }}
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                let fechasTablas = document.querySelectorAll(
                    'th.fecha-tabla'); // aqui tengo el rango de las fechas

                let hilo = datos[0].Hilo;
                let fechaRequerida = datos[0].Fecha_Compromiso; //CAMBIAR POR LA FECHA CORRECTÍSIMA
                ...
                axios.post('/guardar-requerimiento', {
                        ...
                        fechasTablasSemana: fechasTablas,
                        hilo: hilo,
                        fecha_reque = fechaRequerida,
                    }, {
                        headers: {
