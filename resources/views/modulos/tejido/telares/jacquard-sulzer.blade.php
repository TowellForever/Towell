                            ...
                            <tbody>
                                <tr>
                                    <td class="border text-center w-40">
                                        <input type="text" id="julio_reserv" class="border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
    ...
    axios.get('/ultimos-requerimientos')
        .then(response => {
            ...
            requerimientos.forEach(req => {
                ...
                // Convertir fecha del servidor al formato DD-MM-YYYY
                let fechaServidorConvertida = fechaServidor.split('-').reverse().join('-');
                ...
                let fechaTablaEncontrada = Array.from(fechasTablas).find(th => {
                    ...
                    return fechaServidorConvertida === fechaTabla;
                });

                // Si no se encontró una fecha coincidente, no se marcan checkboxes
                if (!fechaTablaEncontrada) return;

                console.log('Fecha coincidente encontrada:', fechaServidorConvertida);
